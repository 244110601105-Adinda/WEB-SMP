...
<head>
    ...
    <title>SMP Negeri 1 Buayan - Prestasi Guru</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .siswa-table tbody tr:nth-child(even) td { background-color: #f9fafb; }
        .siswa-table tbody tr:nth-child(even):hover td { background-color: #f0f8ff; }
...
<?php
// ============================================
// DATA PRESTASI GURU (samakan struktur kolom dengan gambar)
// ============================================
$prestasiGuru = [
    [
        'tp' => '2018/2019',
        'nama_lomba' => 'Guru Berprestasi',
        'jenis' => 'Non Akademik',
        'prestasi' => 'Juara 1',
        'nama_guru' => 'Irwan Slamet Warsito',
        'kab' => '✓',
        'krsdn' => '-',
        'prov' => '-',
        'nas' => '-',
        'int' => '-',
        'keterangan' => 'Kategori Guru Mulja SMP'
    ],
    [
        'tp' => '2018/2019',
        'nama_lomba' => 'Olimpiade Guru Nasional',
        'jenis' => 'Akademik',
        'prestasi' => 'Juara 3',
        'nama_guru' => 'Irwan Slamet Warsito',
        'kab' => '✓',
        ...
        'nas' => '-',
        'int' => '-',
        'keterangan' => 'Kategori Guru IPA SMP'
    ],
    [
        'tp' => '2019/2020',
        'nama_lomba' => 'Olimpiade Guru Nasional',
...
// Filter
$tahunList = ['Semua', '2018/2019', '2019/2020'];
$jenisList = ['Semua', 'Akademik', 'Non Akademik'];

$filterTahun = isset($_GET['tahun']) ? $_GET['tahun'] : 'Semua';
$filterJenis = isset($_GET['jenis']) ? $_GET['jenis'] : 'Semua';

$filteredData = array_filter($prestasiGuru, function($row) use ($filterTahun, $filterJenis) {
    $matchTahun = ($filterTahun === 'Semua' || $row['tp'] === $filterTahun);
    $matchJenis = ($filterJenis === 'Semua' || $row['jenis'] === $filterJenis);
    return $matchTahun && $matchJenis;
});

function badgePrestasiGuru($p) {
    if (strpos($p, 'Juara 1') !== false) return 'badge-1';
    if (strpos($p, 'Juara 2') !== false) return 'badge-2';
    if (strpos($p, 'Juara 3') !== false) return 'badge-3';
    return 'badge-harapan';
}
...
<div class="flex flex-col bg-white">
    <div class="flex flex-col items-start self-stretch bg-[#EAF1F4]">
        <div class="self-stretch">
            ...
            <div class="nav-menu-mobile" id="navMenuMobile">
                ...
                <div class="mobile-dropdown active">
                    ...
                    <div class="mobile-dropdown-content">
                        <a href="prestasi-sekolah.php">Prestasi Sekolah</a>
                        <a href="prestasi-siswa.php">Prestasi Siswa</a>
                        <a href="prestasi-guru.php" style="color:#0090EA;font-weight:600;">Prestasi Guru</a>
                    </div>
                </div>
            ...
            <!-- ============ HALAMAN PRESTASI GURU ============ -->
            <div class="bg-[#EAF1F4] pb-12">
                <!-- Judul PRESTASI GURU ukuran 4xl (text-4xl) -->
                <div class="text-center pt-8 pb-4">
                    <h1 class="text-[#333434] text-4xl font-bold tracking-wider" style="font-family: 'Inria Serif', serif;">PRESTASI GURU</h1>
                </div>
                    ...
                    <form method="GET" action="">
                        <div class="flex flex-wrap gap-3 items-center justify-between mb-6">
                            ...
                            <span class="text-[#636566] text-sm" style="font-family: 'Inria Serif', serif;">
                                Menampilkan <strong><?= count($filteredData) ?></strong> data
                            </span>
                        </div>
                    </form>
                        ...
                        <table class="siswa-table">
                            <thead>
                                <tr>
                                    ...
                                    <th style="width:90px;">TP</th>
                                    <th class="text-left" style="min-width:180px;">NAMA LOMBA</th>
                                    <th style="width:95px;">JENIS</th>
                                    <th style="width:95px;">PRESTASI</th>
                                    <th class="text-left" style="min-width:200px;">NAMA GURU</th>
                                    <th style="width:60px;">KAB</th>
                                    <th style="width:80px;">KRSDN</th>
                                    <th style="width:60px;">PROV</th>
                                    <th style="width:60px;">NAS</th>
                                    <th style="width:55px;">INT</th>
                                    <th class="text-left" style="min-width:220px;">KETERANGAN</th>
                                </tr>
                            </thead>
                            <tbody>
                                <?php if (empty($filteredData)): ?>
                                    <tr class="empty-row">
                                        <td colspan="12">Belum ada data prestasi guru.</td>
                                    </tr>
                                <?php else: foreach ($filteredData as $row): ?>
                                    <tr>
                                        <td><?= $no++ ?></td>
                                        <td><?= htmlspecialchars($row['tp']) ?></td>
                                        <td class="text-left font-medium"><?= htmlspecialchars($row['nama_lomba']) ?></td>
                                        <td><span class="badge badge-jenis"><?= htmlspecialchars($row['jenis']) ?></span></td>
                                        <td><span class="badge <?= badgePrestasiGuru($row['prestasi']) ?>"><?= htmlspecialchars($row['prestasi']) ?></span></td>
                                        <td class="text-left font-semibold"><?= htmlspecialchars($row['nama_guru']) ?></td>

                                        <td class="<?= $row['kab'] === '✓' ? 'text-green-600 font-bold text-base' : 'text-gray-300' ?>"><?= $row['kab'] ?></td>
                                        <td class="<?= $row['krsdn'] === '✓' ? 'text-green-600 font-bold text-base' : 'text-gray-300' ?>"><?= $row['krsdn'] ?></td>
                                        <td class="<?= $row['prov'] === '✓' ? 'text-green-600 font-bold text-base' : 'text-gray-300' ?>"><?= $row['prov'] ?></td>
                                        <td class="<?= $row['nas'] === '✓' ? 'text-green-600 font-bold text-base' : 'text-gray-300' ?>"><?= $row['nas'] ?></td>
                                        <td class="<?= $row['int'] === '✓' ? 'text-green-600 font-bold text-base' : 'text-gray-300' ?>"><?= $row['int'] ?></td>

                                        <td class="text-left text-xs text-[#636566]"><?= htmlspecialchars($row['keterangan']) ?></td>
                                    </tr>
