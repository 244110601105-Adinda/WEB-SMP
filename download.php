...
<head>
    ...
    <title>SMP Negeri 1 Buayan - Download</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .dl-table th.text-left { text-align: left; }
        .dl-table td {
            padding: 12px 18px; border-bottom: 1px solid #e5e5e5;
            color: #333434; font-size: 13px; vertical-align: middle;
            text-align: center;
            font-family: 'Inria Serif', serif;
        }
        .dl-table td.text-left { text-align: left; }
        .dl-table tbody tr:hover td { background-color: #f0f8ff; }

        /* Download link */
        .dl-link {
            color: #0090EA; font-weight: 600; text-decoration: none;
            transition: color 0.2s;
            display: inline-flex; align-items: center; gap: 5px;
            font-family: 'Inria Serif', serif;
        }
        .dl-link:hover { color: #005184; text-decoration: underline; }

        /* Search box */
        .search-input {
            border: 1px solid #d1d5db; border-radius: 20px;
            padding: 7px 16px; font-size: 13px; outline: none;
        ...
        .search-input:focus {
            border-color: #0090EA;
            box-shadow: 0 0 0 2px rgba(0,144,234,0.15);
        }
        ...
        /* Empty row */
        .empty-row td { color: #9ca3af; font-style: italic; padding: 24px; }

        /* Pagination (tetap ada tapi tidak ditampilkan karena data hanya 1 baris) */
        .page-btn {
            padding: 5px 11px; border-radius: 6px; border: 1px solid #ddd;
            background: white; cursor: pointer; font-size: 13px;
            transition: all 0.2s; color: #333434;
            font-family: 'Inria Serif', serif;
        }
        .page-btn:hover, .page-btn.active { background: #0090EA; color: white; border-color: #0090EA; }
...
<body>
    <?php
    // ============================================
    // DATA FILE DOWNLOAD (sesuai gambar: hanya 1 baris contoh)
    // ============================================
    $files = [
        [
            'nama'    => 'Tata Tertib dan Tata Krama',
            'tanggal' => '20 Mei 2026',
            'hits'    => 1797,
            'tipe'    => 'pdf',
            'file'    => 'https://www.smpn1buayan.sch.id/download/hits/TATA_TERTIB_SMPN_1_BUAYAN.pdf',
        ],
    ];

    // Fungsi ikon berdasarkan tipe
    function fileIcon($tipe) {
        switch ($tipe) {
            case 'pdf':   return ['fa-file-pdf',   'icon-pdf'];
            case 'word':  return ['fa-file-word',  'icon-word'];
            case 'excel': return ['fa-file-excel', 'icon-excel'];
            case 'img':   return ['fa-file-image', 'icon-img'];
            case 'zip':   return ['fa-file-zipper','icon-zip'];
            default:      return ['fa-file',        'icon-file'];
        }
    }
    ...
    // Filter by search
    $filtered = array_filter($files, function($f) use ($search) {
        if ($search === '') return true;
        return stripos($f['nama'], $search) !== false;
    });
        ...
        <div class="flex flex-col items-start self-stretch bg-[#EAF1F4]">
            <div class="self-stretch">
                ...
                <div class="bg-white shadow-sm">
                    <div class="desktop-nav">
                        ...
                        <div class="nav-menu-desktop">
                            ...
                            <a href="download.php" style="color:#0090EA;">Download</a>
                        </div>
                    </div>
                </div>
                ...
                <div class="nav-menu-mobile" id="navMenuMobile">
                    ...
                    <a href="download.php" style="color:#0090EA;font-weight:700;">⬇️ Download</a>
                </div>
                ...
                <div class="bg-[#EAF1F4] pb-14">
                    ...
                    <div class="text-center pt-8 pb-4">
                        <h1 class="text-[#333434] text-4xl font-bold tracking-wider" style="font-family: 'Inria Serif', serif;">DOWNLOAD</h1>
                    </div>
                    ...
                    <div class="max-w-5xl mx-auto px-4 mt-10">
                        ...
                        <!-- Search Bar (tetap ada untuk UI) -->
                        <div class="flex justify-end items-center mb-4 gap-3">
                            <label class="text-[#636566] text-sm font-medium" style="font-family: 'Inria Serif', serif;">Search :</label>
                        ...
                        <!-- Tabel (isi sesuai gambar: hanya 1 baris data) -->
                        <div class="table-container bg-white rounded-2xl shadow-lg overflow-hidden">
                            <table class="dl-table">
                                <thead>
                                    <tr>
                                        <th class="text-left" style="width:40%;">NAMA FILE</th>
                                ...
                                <tbody>
                                    ...
                                    <tr class="empty-row">
                                        <td colspan="4">
                                            <?= $search ? 'Tidak ada file yang cocok dengan pencarian "<strong>' . htmlspecialchars($search) . '</strong>".' : 'Belum ada file tersedia.' ?>
                                        </td>
                                    </tr>
                                    <?php else: foreach ($filtered as $f):
                                        [$iconClass, $colorClass] = fileIcon($f['tipe']);
                                    ?>
                                    <tr>
                                        <td class="text-left">
                                            <div class="flex items-center gap-2">
                                                <i class="fa-solid <?= $iconClass ?> <?= $colorClass ?> text-base shrink-0"></i>
                                                <span class="font-medium" style="font-family: 'Inria Serif', serif;"><?= htmlspecialchars($f['nama']) ?></span>
                                            </div>
                                        </td>
                                        <td><?= htmlspecialchars($f['tanggal']) ?></td>
                                        <td>
                                            <span class="hits-badge"><?= number_format($f['hits']) ?></span>
                                        </td>
                                        <td>
                                            <a href="<?= htmlspecialchars($f['file']) ?>"
                                               download
                                               class="dl-link"
                                               onclick="incrementHits(this)">
                        ...
                        <!-- Pagination Info (tetap ada tapi tidak diperlukan karena data hanya 1 baris) -->
                        <?php if ($total > 0): ?>
                        <div class="flex items-center justify-between mt-5 flex-wrap gap-3">
                            <span class="text-[#636566] text-sm" style="font-family: 'Inria Serif', serif;">
                                Menampilkan <?= count($filtered) ?> dari <?= $total ?> file
                                <?= $search ? '(hasil pencarian "<strong>' . htmlspecialchars($search) . '</strong>")' : '' ?>
                            </span>
                        </div>
                        <?php endif; ?>
    ...
    <script>
        ...
        // Simulasi increment hits di sisi client
        function incrementHits(el) {
            const row  = el.closest('tr');
            const badge = row.querySelector('.hits-badge');
            if (badge) {
                const current = parseInt(badge.textContent.replace(/,/g, '')) || 0;
                badge.textContent = (current + 1).toLocaleString('id-ID');
            }
        }
