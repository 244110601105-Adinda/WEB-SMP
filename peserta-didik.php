...
<head>
    ...
    <title>SMP Negeri 1 Buayan - Peserta Didik</title>
    ...
    <style>
        * {
        ...
        /* Table styles - sesuai gambar: kolom TAHUN PELAJARAN, KELAS VII, KELAS VIII, KELAS IX, JUMLAH */
        .peserta-table th {
            background-color: #0090EA;
            color: white;
            padding: 12px 16px;
        ...
        .peserta-table th.text-left-col {
            text-align: left;
        }
        ...
        .peserta-table td.text-left-col {
            text-align: left;
            font-weight: 600;
        }
        ...
        .peserta-table tfoot td {
            background-color: #f3f4f6;
            font-weight: 700;
            border-top: 2px solid #d1d5db;
        }
        
        /* Custom select (tetap untuk filter tahun) */
        .tahun-select {
            background-color: white;
...
<body>
    <?php
    // DATA PESERTA DIDIK - sesuai gambar: hanya total per kelas (tanpa rincian L/P)
    $pesertaDidik = [
        '2023/2024' => [
            'kelas_vii' => 210,
            'kelas_viii' => 207,
    ...
    $tahunList = array_keys($pesertaDidik);
    $selectedTahun = isset($_GET['tahun']) ? $_GET['tahun'] : '2025/2026';
    if (!isset($pesertaDidik[$selectedTahun])) {
        $selectedTahun = '2025/2026';
    }
    
    // Data untuk semua tahun (tabel lengkap)
    $allData = [];
    foreach ($pesertaDidik as $tahun => $data) {
        $allData[] = [
            'tahun' => $tahun,
            'kelas_vii' => $data['kelas_vii'],
            'kelas_viii' => $data['kelas_viii'],
            'kelas_ix' => $data['kelas_ix'],
            'jumlah' => $data['kelas_vii'] + $data['kelas_viii'] + $data['kelas_ix']
        ];
    }
    ?>
        ...
        <div class="flex flex-col items-start self-stretch bg-[#EAF1F4]">
            <div class="self-stretch">
                ...
                <div class="bg-white shadow-sm">
                    <div class="desktop-nav">
                        ...
                        <div class="nav-menu-desktop">
                            ...
                            <a href="peserta-didik.php" style="color:#0090EA; font-family: 'Inria Serif', serif;">Peserta Didik</a>
                ...
                <div class="nav-menu-mobile" id="navMenuMobile">
                    ...
                    <a href="peserta-didik.php" style="color:#0090EA; font-weight:700;">👨‍🎓 Peserta Didik</a>
                ...
                <!-- ============================================ -->
                <!-- HALAMAN PESERTA DIDIK (tanpa card tambahan, tabel sesuai gambar) -->
                <!-- ============================================ -->
                <div class="bg-[#EAF1F4] pb-12">
                    
                    <!-- Judul PESERTA DIDIK ukuran 4xl (text-4xl) -->
                    <div class="text-center pt-8 pb-4">
                        <h1 class="text-[#333434] text-4xl font-bold tracking-wider" style="font-family: 'Inria Serif', serif;">PESERTA DIDIK</h1>
                    </div>
                    ...
                    <!-- Konten (tabel sesuai gambar) -->
                    <div class="max-w-5xl mx-auto px-4 mt-10">
                        
                        <!-- Tabel Peserta Didik - hanya kolom: TAHUN PELAJARAN, KELAS VII, KELAS VIII, KELAS IX, JUMLAH -->
                        <div class="table-container bg-white rounded-2xl shadow-lg overflow-hidden">
                            <table class="peserta-table w-full border-collapse">
                                <thead>
                                    <tr>
                                        <th class="text-left-col" style="min-width:180px;">TAHUN PELAJARAN</th>
                                        <th>KELAS VII</th>
                                        <th>KELAS VIII</th>
                                        <th>KELAS IX</th>
                                        <th>JUMLAH</th>
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td class="text-left-col font-semibold"><?= htmlspecialchars($row['tahun']) ?></td>
                                        <td class="font-medium"><?= number_format($row['kelas_vii']) ?></td>
                                        <td class="font-medium"><?= number_format($row['kelas_viii']) ?></td>
                                        <td class="font-medium"><?= number_format($row['kelas_ix']) ?></td>
                                        <td class="font-bold text-[#0090EA]"><?= number_format($row['jumlah']) ?></td>
                                    </tr>
                                    <?php endforeach; ?>
                                </tbody>
                                <tfoot>
                                    <?php
                                    $totalVII = array_sum(array_column($allData, 'kelas_vii'));
                                    $totalVIII = array_sum(array_column($allData, 'kelas_viii'));
                                    $totalIX = array_sum(array_column($allData, 'kelas_ix'));
                                    $totalSemua = $totalVII + $totalVIII + $totalIX;
                                    ?>
                                    <tr style="background-color: #f3f4f6; border-top: 2px solid #d1d5db;">
                                        <td class="font-bold text-left-col">TOTAL KESELURUHAN</td>
                                        <td class="font-bold"><?= number_format($totalVII) ?></td>
                                        <td class="font-bold"><?= number_format($totalVIII) ?></td>
                                        <td class="font-bold"><?= number_format($totalIX) ?></td>
                                        <td class="font-bold text-[#0090EA]"><?= number_format($totalSemua) ?></td>
                                    </tr>
                                </tfoot>
                            </table>
                        </div>
                ...
                <!-- ============================================ -->
                <!-- FOOTER (Font Inria Serif, tidak diubah) -->
                <!-- ============================================ -->
    ...
    <script>
        function showToast(message) {
            ...
            toast.style.cssText = 'position:fixed;bottom:20px;right:20px;background:#333;color:white;padding:10px 20px;border-radius:8px;z-index:9999;font-size:13px;font-family:"Inria Serif",serif;';
